...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if self.training and getattr(self.data_processor, 'only_keep_gt_box_scenes', False):
            infos = []
            for info in self.infos:
                mask = self.data_processor.gt_box_mask[info['frame_id']]
                if mask.sum() > 0:
                    infos.append(info)
            self.infos = infos
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            if self.training and hasattr(self.data_processor, 'gt_box_mask'):
                ...
                gt_box_mask = self.data_processor.gt_box_mask[info['frame_id']]
                if mask is not None:
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
                                    processed_data_tag=None, gt_valid_mask_dict=None):
        ...
        for k in tqdm(range(0, len(infos), sampled_interval)):
            ...
            if gt_valid_mask_dict is not None:
                gt_valid_mask = gt_valid_mask_dict[info['frame_id']]
...
def create_waymo_gt_database_with_mask(
    dataset_cfg, class_names, data_path, save_path, train_filename, gt_valid_mask_filename, processed_data_tag='waymo_processed_data'):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
    )
    train_split = 'train'
    train_filename = save_path / train_filename
    print('---------------Start create groundtruth database for data augmentation---------------')
    dataset.set_split(train_split)
    ...
    dataset.create_groundtruth_database(
        info_path=train_filename, save_path=save_path, split='train', sampled_interval=1,
        used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag, gt_valid_mask_dict=gt_valid_mask_dict
    )
    print('---------------Data preparation Done---------------')
