...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        instance_idx_base = 0
        if 'weak_clusters_label' in data_dict and data_dict['weak_source'] == 'panoptic':
            if (points[:, -2] < 65535).any():
                instance_idx_base = points[:, -2][points[:, -2] < 65535].max() + 1
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if data_dict.get('weak_source', None) == 'panoptic':
                padding_panoptic_labels = np.zeros((obj_points.shape[0], 2))
                padding_panoptic_labels[:, 0] = instance_idx_base + idx
                padding_panoptic_labels[:, 1] = self.class_names.index(info['name']) + 1

                # if sampled labels will not be used in weakly supervised training
                # padding_panoptic_labels[:, 0] = 65535
                obj_points = np.concatenate([obj_points, padding_panoptic_labels], axis=1)
        ...
        points, keep_mask = box_utils.remove_points_in_boxes3d(points, large_sampled_gt_boxes, return_mask=True)
        ...
        if 'weak_clusters_label' in data_dict and 'instance_ID_of_point' in data_dict['weak_clusters_label']:
            ...
            instance_ID_of_point = instance_ID_of_point[keep_mask]
            instance_ID_of_point = np.concatenate([np.zeros(len(obj_points), dtype=np.int64), instance_ID_of_point])
            data_dict['weak_clusters_label']['instance_ID_of_point'] = instance_ID_of_point
    ...
    def __call__(self, data_dict):
        ...
        if data_dict.get('weak_clusters_label', None) is not None:
            weak_clusters_label = data_dict['weak_clusters_label']
            existed_circles = np.concatenate([weak_clusters_label['center_of_cluster'][:, :2], weak_clusters_label['radius_of_cluster'][:, None]], axis=1)
        else:
            existed_circles = None
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if existed_circles is not None and len(existed_circles) > 0:
                    collision = iou3d_nms_utils.boxes_circles_collision_check_cpu(sampled_boxes[:, 0:7], existed_circles)
                    valid_mask = valid_mask & (collision.max(axis=1) == 0)
