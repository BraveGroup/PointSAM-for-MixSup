...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            gt_boxes_mask = np.array([n in self.class_names for n in data_dict['gt_names']], dtype=bool)
            if 'weak_clusters_label' in data_dict and 'name_of_cluster' in data_dict['weak_clusters_label']:
                mask = np.array([n in self.class_names for n in data_dict['weak_clusters_label']['name_of_cluster']], dtype=bool)
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            if 'weak_clusters_label' in data_dict and 'name_of_cluster' in data_dict['weak_clusters_label']:
                selected = common_utils.keep_arrays_by_name(data_dict['weak_clusters_label']['name_of_cluster'], self.class_names)
                for key in data_dict['weak_clusters_label']:
                    if key.endswith('of_cluster'):
                        data_dict['weak_clusters_label'][key] = data_dict['weak_clusters_label'][key][selected]
                if 'instance_ID_of_point' in data_dict['weak_clusters_label']:
                    instance_ID_of_point = data_dict['weak_clusters_label']['instance_ID_of_point']
                    remapping = np.zeros(len(data_dict['weak_clusters_label']['name_of_cluster']) + 1, dtype=np.int64)
                    remapping[selected + 1] = np.arange(len(selected)) + 1
                    data_dict['weak_clusters_label']['instance_ID_of_point'] = remapping[instance_ID_of_point]
                labels = np.array([self.class_names.index(n) + 1 for n in data_dict['weak_clusters_label']['name_of_cluster']], dtype=np.int32)
                data_dict['weak_clusters_label']['label_of_cluster'] = labels
        ...
        data_dict.pop('weak_source', None)
        return data_dict
