...
class PointFeatureEncoder(object):
    ...
    def forward(self, data_dict):
        ...
        data_dict['points'], use_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
            data_dict['points'],
            data_dict.get('temp_src_feature_list', []),
            data_dict.get('temp_used_feature_list', [])
        )
    ...
    def absolute_coordinates_encoding(self, points=None, temp_src_feature_list=[], temp_used_feature_list=[]):
        if points is None:
            num_output_features = len(self.used_feature_list)
            return num_output_features

        src_feature_list = self.src_feature_list + temp_src_feature_list
        used_feature_list = self.used_feature_list + temp_used_feature_list
        assert points.shape[-1] == len(src_feature_list)
