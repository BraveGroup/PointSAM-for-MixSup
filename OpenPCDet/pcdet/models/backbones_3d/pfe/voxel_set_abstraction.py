...
def sector_fps(points, num_sampled_points, num_sectors, points_idx=None):
    """
    Args:
        points: (N, 3)
    ...
    if points_idx is None:
        points_idx = torch.arange(points.shape[0], device=points.device).long()
        return_idx = False
    else:
        return_idx = True
    ...
    for k in range(num_sectors):
        ...
        if cur_num_points > 0:
            ...
            xyz_points_idx_list.append(points_idx[mask])
    ...
    if len(xyz_batch_cnt) == 0:
        ...
        xyz_points_idx_list.append(points_idx)
    ...
    xyz_idx = torch.cat(xyz_points_idx_list, dim=0)
    ...
    sampled_idx = xyz_idx[sampled_pt_idxs]
...
class VoxelSetAbstraction(nn.Module):
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        sampled_point_idx = torch.nonzero(point_mask).view(-1)
        sampled_points, sampled_point_idx = sector_fps(
            points=sampled_points, num_sampled_points=self.model_cfg.NUM_KEYPOINTS,
            num_sectors=self.model_cfg.SPC_SAMPLING.NUM_SECTORS, points_idx=sampled_point_idx
        )
        return sampled_points, sampled_point_idx
    ...
    def get_sampled_points(self, batch_dict):
        ...
        keep_instance_ID = True if 'gt_mixed_label' in batch_dict else False
        ...
        keypoints_list = []
        keypoints_instance_ID_list = [] if keep_instance_ID else None
        for bs_idx in range(batch_size):
            ...
            if self.model_cfg.SAMPLE_METHOD == 'FPS':
                ...
                if keep_instance_ID:
                    keypoints_instance_ID_list.append(
                        batch_dict['gt_mixed_label'][bs_idx]['Clusters']['instance_ID_of_point'][cur_pt_idxs[0]]
                    )

            elif self.model_cfg.SAMPLE_METHOD == 'SPC':
                cur_keypoints, cur_pt_idxs = self.sectorized_proposal_centric_sampling(
                ...
                if keep_instance_ID:
                    keypoints_instance_ID_list.append(
                        batch_dict['gt_mixed_label'][bs_idx]['Clusters']['instance_ID_of_point'][cur_pt_idxs]
                    )
            else:
                raise NotImplementedError
        ...
        if keep_instance_ID:
            keep_instance_ID = torch.cat(keypoints_instance_ID_list, dim=0)
            batch_dict['instance_ID_of_keypoint'] = keep_instance_ID
