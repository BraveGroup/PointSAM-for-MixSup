...
inline int check_box_circle_collision(const float *box, const float *circle){
    // params: box (7) [x, y, z, dx, dy, dz, heading]
    // params: circle (3) [x, y, r]
    Point circle_center(circle[0], circle[1]);
    Point box_center(box[0], box[1]);
    float radius = circle[2];
    float angle_cos = cos(-box[6]), angle_sin = sin(-box[6]);  // rotate the center of circle in the opposite direction of box
    rotate_around_center(box_center, angle_cos, angle_sin, circle_center);
    float dx_half = box[3] / 2, dy_half = box[4] / 2;
    float delta_x = fabs(circle_center.x - box_center.x), delta_y = fabs(circle_center.y - box_center.y);
    if (delta_x > dx_half + radius || delta_y > dy_half + radius) return 0;
    if (delta_x <= dx_half || delta_y <= dy_half) return 1;
    float dx = delta_x - dx_half, dy = delta_y - dy_half;    
...
int boxes_circles_collision_check_cpu(at::Tensor boxes_tensor, at::Tensor circles_tensor, at::Tensor ans_tensor){
    // params boxes_tensor: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params circles_tensor: (M, 3) [x, y, r]

    CHECK_CONTIGUOUS(boxes_tensor);
    CHECK_CONTIGUOUS(circles_tensor);

    int num_boxes = boxes_tensor.size(0);
    int num_circles = circles_tensor.size(0);
    const float *boxes = boxes_tensor.data<float>();
    const float *circles = circles_tensor.data<float>();
    int *ans = ans_tensor.data<int>();

    for (int i = 0; i < num_boxes; i++){
        for (int j = 0; j < num_circles; j++){
            ans[i * num_circles + j] = check_box_circle_collision(boxes + i * 7, circles + j * 3);
        }
    }
    return 1;
}
