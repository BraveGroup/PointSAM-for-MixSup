...
def boxes_circles_collision_check_cpu(boxes, circles):
    """
    Args:
        boxes: (N, 7) [x, y, z, dx, dy, dz, heading]
        circles: (M, 3) [x, y, radius]

    Returns:
        ans_collision: (N, M)
    """
    boxes, is_numpy = common_utils.check_numpy_to_torch(boxes)
    circles, is_numpy = common_utils.check_numpy_to_torch(circles)
    assert not (boxes.is_cuda or circles.is_cuda), 'Only support CPU tensors'
    assert boxes.shape[1] == 7 and circles.shape[1] == 3
    ans_collision = torch.zeros((boxes.shape[0], circles.shape[0]), dtype=torch.int32)
    iou3d_nms_cuda.boxes_circles_collision_check_cpu(boxes.contiguous(), circles.contiguous(), ans_collision)
    return ans_collision.numpy() if is_numpy else ans_collision
