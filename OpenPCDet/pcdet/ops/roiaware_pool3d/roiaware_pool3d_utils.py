...
def points_in_multi_boxes_gpu(points, boxes, max_box_num=-1):
    """
    One point may be in multiple boxes, this function returns the box indices for each point
    :param points: (B, M, 3)
    :param boxes: (B, T, 7), num_valid_boxes <= T
    :param max_box_num: int, if > 0, only consider the first max_box_num boxes for each point
    :return box_idxs_of_pts: (B, M, max_box_num), default background = -1
    """
    assert boxes.shape[0] == points.shape[0]
    assert boxes.shape[2] == 7 and points.shape[2] == 3
    batch_size, num_points, _ = points.shape
    num_boxes = boxes.shape[1]
    max_box_num = num_boxes if max_box_num < 0 else max_box_num

    box_idxs_of_pts = points.new_zeros((batch_size, num_points, max_box_num),
                                    dtype=torch.int).fill_(-1)
    roiaware_pool3d_cuda.points_in_multi_boxes_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)

    return box_idxs_of_pts


def points_in_multi_aligned_bev_boxes_gpu(points, boxes, max_box_num=-1):
    """
    One point may be in multiple boxes, this function returns the box indices for each point
    :param points: (B, M, 3)
    :param boxes: (B, T, 4), num_valid_boxes <= T
    :param max_box_num: int, if > 0, only consider the first max_box_num boxes for each point
    :return box_idxs_of_pts: (B, M, max_box_num), default background = -1
    """
    assert boxes.shape[0] == points.shape[0]
    assert boxes.shape[2] == 4 and points.shape[2] == 3
    batch_size, num_points, _ = points.shape
    num_boxes = boxes.shape[1]
    max_box_num = num_boxes if max_box_num < 0 else max_box_num
    ...
    box_idxs_of_pts = points.new_zeros((batch_size, num_points, max_box_num),
                                    dtype=torch.int).fill_(-1)
    roiaware_pool3d_cuda.points_in_multi_aligned_bev_boxes_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)
