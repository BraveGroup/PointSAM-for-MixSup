...
int points_in_multi_boxes_gpu(at::Tensor boxes_tensor, at::Tensor pts_tensor,
...
//   CHECK_INPUT(box_idx_of_points_tensor);
  ...
  int boxes_num = boxes_tensor.size(1);
  int pts_num = pts_tensor.size(1);
  int max_box_num = box_idx_of_points_tensor.size(2);
  ...
  points_in_multi_boxes_launcher(batch_size, boxes_num, pts_num, max_box_num,
...
int points_in_multi_aligned_bev_boxes_gpu(at::Tensor boxes_tensor, at::Tensor pts_tensor,
                                          at::Tensor box_idx_of_points_tensor) {
...
//   CHECK_INPUT(box_idx_of_points_tensor);
  ...
  points_in_multi_aligned_bev_boxes_launcher(batch_size, boxes_num, pts_num, max_box_num,
                                             boxes, pts, box_idx_of_points);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("points_in_multi_aligned_bev_boxes_gpu", &points_in_multi_aligned_bev_boxes_gpu, "points_in_multi_aligned_bev_boxes_gpu forward (CUDA)");
}
