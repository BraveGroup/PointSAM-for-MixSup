...
__device__ inline int check_pt_in_aligned_bev_boxes(const float *pt, const float *box){
    // param pt: (x, y, z)
...
__global__ void points_in_multi_boxes_kernel(int batch_size, int boxes_num, int pts_num, int max_box_num,
    const float *boxes, const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z] in LiDAR coordinate
    // params boxes_idx_of_points: (B, npoints, max_box_num), default -1
    ...
    boxes += bs_idx * boxes_num * 7;
    pts += bs_idx * pts_num * 3 + pt_idx * 3;
    box_idx_of_points += bs_idx * pts_num * max_box_num + pt_idx * max_box_num;

    float local_x = 0, local_y = 0;
    int cur_in_flag = 0;
    int cur_box_idx = 0;
    for (int k = 0; k < boxes_num && cur_box_idx < max_box_num; k++){
        cur_in_flag = check_pt_in_box3d(pts, boxes + k * 7, local_x, local_y);
        if (cur_in_flag){
            box_idx_of_points[cur_box_idx] = k;
            cur_box_idx++;
        }
    }
}
...
void points_in_multi_boxes_launcher(int batch_size, int boxes_num, int pts_num, int max_box_num,
    const float *boxes, const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params boxes_idx_of_points: (B, npoints, max_box_num), default -1
    ...
    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_in_multi_boxes_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, max_box_num, boxes, pts, box_idx_of_points);
...
__global__ void points_in_multi_aligned_bev_boxes_kernel(int batch_size, int boxes_num, int pts_num, int max_box_num,
    const float *boxes, const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 4) [x1, y1, x2, y2]
    // params pts: (B, npoints, 3) [x, y, z] in LiDAR coordinate
    // params boxes_idx_of_points: (B, npoints, max_box_num), default -1
    ...
    int cur_box_idx = 0;
    for (int k = 0; k < boxes_num && cur_box_idx < max_box_num; k++){
        cur_in_flag = check_pt_in_aligned_bev_boxes(pts, boxes + k * 4);
        if (cur_in_flag){
            box_idx_of_points[cur_box_idx] = k;
            cur_box_idx++;
        }
...
void points_in_multi_aligned_bev_boxes_launcher(int batch_size, int boxes_num, int pts_num, int max_box_num,
    const float *boxes, const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 4) [x1, y1, x2, y2]
    // params pts: (B, npoints, 3) [x, y, z]
    // params boxes_idx_of_points: (B, npoints, max_box_num), default -1
    ...
    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_in_multi_aligned_bev_boxes_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, max_box_num, boxes, pts, box_idx_of_points);
